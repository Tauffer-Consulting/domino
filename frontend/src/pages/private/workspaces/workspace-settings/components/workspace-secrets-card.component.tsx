...
const WorkspaceSecretsCard = () => {
    ...
    const handleSaveToken = useCallback(async(e: any) => {
        e.preventDefault();
        ...
        if (e.currentTarget.ariaLabel === 'clear') {
            ...
            patchWorkspace({
                ...
                resetField(`github-token-workspace-${workspace?.id}`, { keepTouched: false })
            }).catch((err) => {
    ...
    }, [workspace, getValues, patchWorkspace, resetField, handleUpdateWorkspace, defaultValue]);

    if (!workspace) {
        return (
            <h1>Workspace not selected.</h1>
        )
    }
    return (
            ...
            <CardContent>
                <Box>
                    <Typography variant='body1'>
                        Workspace github token is used to access private pieces repositories.
                        Furhtermore, authenticated requests get a higher rate limit from Github.
                    </Typography>
                </Box>
                <form>
                    <Grid item xs={12} container spacing={2} sx={{ marginTop: '15px' }}>
                        <Grid item xs={7} sm={8} md={10}>
                            <TextField
                                ...
                                defaultValue={workspace.github_access_token_filled ? "******" : ""}
                                type="password"
                                fullWidth
                                ...
                                currentEdittingToken ? (
                                    <div>
                                        ...
                                        <Tooltip title="Cancel">
                                            <IconButton aria-label="cancel" onClick={() => setCurrentEdittingToken(false)}>
                                                <CancelIcon />
                                            </IconButton>
                                        </Tooltip>
                                    </div>
                                    ...
                                    <Tooltip title="Edit">
                                            <IconButton aria-label="edit" onClick={() => setCurrentEdittingToken(true)}>
                                            <EditIcon />
                                        </IconButton>
                                    </Tooltip>
                                )
