...
const getWorkflowRunTasks: (
    ...
    page: number,
    pageSize: number
) => Promise<AxiosResponse<IGetWorkflowRunTasksResponseInterface>> = (workspace, workflowId, runId, page, pageSize) => {
    return dominoApiClient.get(getWorkflowRunTasksUrl(workspace, workflowId, runId, page, pageSize))
}
...
export const useAuthenticatedGetWorkflowRunTasks = () => {
    ...
    return (params: IGetWorkflowRunTasksParams,) => getWorkflowRunTasks(
        workspace.id,
        params.workflowId,
        params.runId,
        params.page,
