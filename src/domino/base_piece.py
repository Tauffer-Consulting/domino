...
class BasePiece(metaclass=abc.ABCMeta):
    ...
    def run_piece_function(
        ...
        self.storage_size_in_bytes = self._get_folder_size(base_results_path)
        self.logger.info(f"Piece used {self.storage_size_in_bytes} bytes of storage.")
